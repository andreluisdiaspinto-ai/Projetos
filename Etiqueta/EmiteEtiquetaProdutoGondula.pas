unit EmiteEtiquetaProdutoGondula;

{ Gerador de Etiquetas de Produto - padrao de gondola de supermercado.

  Baseado no RelatorioProduto (mesmas regras de filtro e o mesmo fluxo de
  ...
  atualiza a DBGridProduto. O botao Imprimir gera as etiquetas via
  FastReport (layout guardado na tabela RELATORIO, campo BLOB ARQUIVO),
  uma etiqueta por produto, dispostas em duas colunas na folha A4.

  Cada etiqueta exibe:
    - Titulo "Empresa: Ganso Demonstracao Ltda.";
    - DESCRICAO ao lado do PRECO_VENDA;
    - CODIGO_BARRAS (numero) e, abaixo, o codigo de barras EAN13. }
...
const
  ...
  // alimentar a tabela.
  NOME_RELATORIO_ETIQUETA = 'ETIQUETA_PRODUTO_GONDULA';
  ARQUIVO_FR3             = 'EtiquetaProdutoGondula.fr3';
  PASTA_FR3               = 'C:\Avaliacao Delphi_Firebird\Andre_luis\Projeto\Fr3';
  PASTA_FR3_RELATIVA      = 'Fr3';

type
  TForm_EmiteEtiquetaProdutoGondula = class(TForm)
    Panel_Titulo: TPanel;
...
var
  Form_EmiteEtiquetaProdutoGondula: TForm_EmiteEtiquetaProdutoGondula;
...
{ Utilitarios visuais (mesmo padrao do RelatorioProduto) }

procedure TForm_EmiteEtiquetaProdutoGondula.ConfigurarGlyphsBotoes;
begin
...
procedure TForm_EmiteEtiquetaProdutoGondula.FormCreate(Sender: TObject);
begin
  Color := COR_PAGE;
...
procedure TForm_EmiteEtiquetaProdutoGondula.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  QueryProduto.Close;
  Action := caFree;
  Form_EmiteEtiquetaProdutoGondula := nil;
end;

{ Navegacao por teclado }

procedure TForm_EmiteEtiquetaProdutoGondula.FormKeyDown(Sender: TObject;
...
procedure TForm_EmiteEtiquetaProdutoGondula.EditFocoEnter(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_FOCO;
end;

procedure TForm_EmiteEtiquetaProdutoGondula.EditFocoExit(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_NORMAL;
end;

procedure TForm_EmiteEtiquetaProdutoGondula.EditNumericoKeyPress(
...
procedure TForm_EmiteEtiquetaProdutoGondula.DBGridProdutoDrawColumnCell(
...
function TForm_EmiteEtiquetaProdutoGondula.ValidarFaixaCodigo(out ACodigoDe,
...
procedure TForm_EmiteEtiquetaProdutoGondula.AjustarFormatoCampos;
...
procedure TForm_EmiteEtiquetaProdutoGondula.AtualizarProgresso(
  Percentual: Integer);
begin
...
procedure TForm_EmiteEtiquetaProdutoGondula.DestacarColunaOrdenacao;
var
  I: Integer;
  CampoOrdem: string;
begin
...
procedure TForm_EmiteEtiquetaProdutoGondula.Pesquisar;
var
  Where, Sql: string;
...
begin
  ...
  Sql := 'select CODIGO, DESCRICAO, REFERENCIA, MARCA, GRUPO, ' +
         'PRECO_VENDA, ESTOQUE_ATUAL, CODIGO_BARRAS' + Where;
  if RadioGrupoOrdem.ItemIndex = 1 then
...
procedure TForm_EmiteEtiquetaProdutoGondula.BtnPesquisarClick(Sender: TObject);
begin
  Pesquisar;
...
procedure TForm_EmiteEtiquetaProdutoGondula.BtnLimparClick(Sender: TObject);
begin
...
function TForm_EmiteEtiquetaProdutoGondula.LocalizarArquivoFr3: string;
var
  PastaExe: string;
...
procedure TForm_EmiteEtiquetaProdutoGondula.GravarRelatorioNoBanco(const ANome,
...
function TForm_EmiteEtiquetaProdutoGondula.CarregarRelatorioDoBanco(
...
procedure TForm_EmiteEtiquetaProdutoGondula.ExibirRelatorio;
var
...
begin
  ...
  try
    ...
    if not CarregarRelatorioDoBanco(NOME_RELATORIO_ETIQUETA, Stream) then
    begin
      CaminhoFr3 := LocalizarArquivoFr3;
      if CaminhoFr3 = '' then
      begin
        MensagemDlg('Layout "' + NOME_RELATORIO_ETIQUETA + '" nao ' +
          'encontrado na tabela RELATORIO e o arquivo ' + ARQUIVO_FR3 +
          ' nao foi localizado em disco.', mtWarning, [mbOK], 0);
        Exit;
      end;

      GravarRelatorioNoBanco(NOME_RELATORIO_ETIQUETA, CaminhoFr3);
      if not CarregarRelatorioDoBanco(NOME_RELATORIO_ETIQUETA, Stream) then
      begin
...
procedure TForm_EmiteEtiquetaProdutoGondula.BtnImprimirClick(Sender: TObject);
begin
  if not QueryProduto.Active or QueryProduto.IsEmpty then
  begin
    MensagemDlg('Nao ha produtos para gerar etiquetas. Utilize o botao ' +
...
procedure TForm_EmiteEtiquetaProdutoGondula.BtnSairClick(Sender: TObject);
begin
  Close;
end;
