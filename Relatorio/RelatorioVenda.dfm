object Form_RelatorioVenda: TForm_RelatorioVenda
  Left = 0
  Top = 0
  BorderIcons = []
  BorderStyle = bsNone
  ...
  object DBGridVenda: TDBGrid
    ...
    TitleFont.Style = []
    OnDrawColumnCell = DBGridVendaDrawColumnCell
    Columns = <
      item
        Expanded = False
        FieldName = 'CODIGO'
  ...
  object QueryVenda: TIBQuery
    ...
    ParamCheck = True
    SQL.Strings = (
      'select V.CODIGO, V.CODIGO_CLIENTE, C.NOME as NOME_CLIENTE,'
      '       V.DATA_HORA_VENDA, V.TOTAL_BRUTO, V.DESCONTO_PERC,'
      '       V.ACRESCIMO_PER, V.TOTAL_LIQUIDO'
      'from VENDA V'
      'left join CLIENTE C on C.CODIGO = V.CODIGO_CLIENTE'
      'order by C.NOME, V.CODIGO')
    Left = 24
    Top = 495
  end
