unit RelatorioVendaSinteticaCliente;

{ Relatorio de Venda Sintetica por Cliente (uma linha por venda).

  Consulta sintetica com um unico join:

    VENDA  +  CLIENTE (nome e telefone do cliente)

  Uma linha por venda (sem VENDA_ITEM/PRODUTO), agrupada por cliente.

  Filtros disponiveis:
    1 - Ordem do resultado: fixa por Nome do cliente;
  ...
  Quando ambos "codigo" e "texto" estao preenchidos para cliente, o
  codigo tem preferencia (mais especifico).
  ...
  Quebras/subtotais do relatorio:
    - Quebra por cliente (NOME_CLIENTE).
    - Subtotal por cliente: soma TOTAL_BRUTO e TOTAL_LIQUIDO das vendas.
    - Total geral no rodape.
    - Ordenacao fixa: Nome do cliente.
...
const
  ...
  NOME_RELATORIO_VENDA_CLIENTE = 'Relatorio_Venda_Sintetica_Cliente';
  ARQUIVO_FR3                    = 'RelatorioVendaSinteticaCliente.fr3';
  PASTA_FR3          = 'C:\Avaliacao Delphi_Firebird\Andre_luis\Projeto\Fr3';
  PASTA_FR3_RELATIVA = 'Fr3';

type
  TForm_RelatorioVendaSinteticaCliente = class(TForm)
    Panel_Titulo: TPanel;
...
var
  Form_RelatorioVendaSinteticaCliente: TForm_RelatorioVendaSinteticaCliente;
...
procedure TForm_RelatorioVendaSinteticaCliente.ConfigurarGlyphsBotoes;
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.FormCreate(Sender: TObject);
var
  lCaminhoFr3: string;
begin
  Color := COR_PAGE;
...
procedure TForm_RelatorioVendaSinteticaCliente.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  QueryVendaCliente.Close;
  QueryGraficoPorCliente.Close;
  Action := caFree;
  Form_RelatorioVendaSinteticaCliente := nil;
end;

{ Navegacao por teclado }

procedure TForm_RelatorioVendaSinteticaCliente.FormKeyDown(Sender: TObject;
...
begin
  ...
  begin
    ...
    // ultimo campo (Nome Cliente) esta focado, senao segue ao proximo.
...
procedure TForm_RelatorioVendaSinteticaCliente.EditFocoEnter(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_FOCO;
end;

procedure TForm_RelatorioVendaSinteticaCliente.EditFocoExit(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_NORMAL;
end;

procedure TForm_RelatorioVendaSinteticaCliente.EditNumericoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.EditDataKeyPress(Sender: TObject;
  var Key: Char);
var
  lEdit: TEdit;
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.DBGridVendaClienteDrawColumnCell(
...
procedure TForm_RelatorioVendaSinteticaCliente.BtnLupaClienteClick(Sender: TObject);
var
  lCodigo: Integer;
  lNome: string;
begin
...
function TForm_RelatorioVendaSinteticaCliente.ValidarFaixaCodigo(out ACodigoDe,
...
function TForm_RelatorioVendaSinteticaCliente.ValidarFaixaData(out ADataDe,
...
procedure TForm_RelatorioVendaSinteticaCliente.AjustarFormatoCampos;
...
begin
  // Zeros a esquerda: venda com 6 casas, cliente com 5 casas.
...
procedure TForm_RelatorioVendaSinteticaCliente.AtualizarProgresso(APercentual: Integer);
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.DestacarColunaOrdenacao;
var
  lI: Integer;
begin
  // Destaque na coluna da ordenacao fixa: Cliente (Nome).
  for lI := 0 to DBGridVendaCliente.Columns.Count - 1 do
    if SameText(DBGridVendaCliente.Columns[lI].FieldName, 'NOME_CLIENTE') then
    begin
...
procedure TForm_RelatorioVendaSinteticaCliente.Pesquisar;
var
  lWhere, lSql: string;
...
begin
  ...
  // SELECT sintetico: uma linha por venda, com nome e telefone do cliente.
  // FastReport agrupa/subtotaliza por NOME_CLIENTE (ver quebra no .fr3).
  lSql :=
    'select' +
    '  vc.CODIGO,' +
    '  vc.DATA_HORA_VENDA,' +
  ...
  // Venda Sintetica por Cliente: ordenacao fixa por Nome (necessaria para a quebra).
  lSql := lSql +
...
procedure TForm_RelatorioVendaSinteticaCliente.BtnPesquisarClick(Sender: TObject);
begin
  Pesquisar;
...
procedure TForm_RelatorioVendaSinteticaCliente.BtnLimparClick(Sender: TObject);
begin
...
function TForm_RelatorioVendaSinteticaCliente.LocalizarArquivoFr3: string;
var
  lPastaExe: string;
...
procedure TForm_RelatorioVendaSinteticaCliente.GravarRelatorioNoBanco(const ANome,
  ...
  try
    ...
    try
      ...
      lQuery.ParamByName('PDESCRICAO').AsString :=
        'Relatorio de Venda Sintetica por Cliente';
...
procedure TForm_RelatorioVendaSinteticaCliente.GravarRelatorioEmbutidoNoBanco(
  const ANome: string);
var
  lQuery: TIBQuery;
...
procedure TForm_RelatorioVendaSinteticaCliente.SincronizarLayoutFr3NoBanco;
var
  lCaminhoFr3: string;
begin
  lCaminhoFr3 := LocalizarArquivoFr3;
...
function TForm_RelatorioVendaSinteticaCliente.CarregarRelatorioDoBanco(
...
function TForm_RelatorioVendaSinteticaCliente.DescricaoFiltros: string;
...
procedure TForm_RelatorioVendaSinteticaCliente.AplicarCabecalhoRelatorio;
var
  lMemo: TfrxMemoView;
  lTitle: TfrxBand;
  lI: Integer;
...
begin
  ...
  begin
    ...
    lMemo.Text := 'Relat'#243'rio de Venda Sint'#233'tica por Cliente';
  end;
end;

procedure TForm_RelatorioVendaSinteticaCliente.ConfigurarGraficoPizzaPorCliente;
var
...
procedure TForm_RelatorioVendaSinteticaCliente.ExibirRelatorio;
var
  lStream: TMemoryStream;
  lCaminhoFr3: string;
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.BtnImprimirClick(Sender: TObject);
begin
...
procedure TForm_RelatorioVendaSinteticaCliente.BtnSairClick(Sender: TObject);
begin
  Close;
end;
