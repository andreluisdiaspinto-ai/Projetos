unit RelatorioCliente;

{ Filtro para Relatorio de Clientes.
  ...
  atualiza a DBGridCliente. A impressao sera feita pelo componente
  FastReport, usando a QueryCliente como fonte de dados. }
...
const
  ...
  // alimentar a tabela.
  NOME_RELATORIO_CLIENTE = 'RELATORIO_CLIENTE';
  ARQUIVO_FR3            = 'Relatorio_Cliente.fr3';
  PASTA_FR3              = 'C:\Avaliacao Delphi_Firebird\Andre_luis\Projeto\Fr3';
  PASTA_FR3_RELATIVA     = 'Fr3';

type
  TForm_RelatorioCliente = class(TForm)
    Panel_Titulo: TPanel;
...
var
  Form_RelatorioCliente: TForm_RelatorioCliente;
...
procedure TForm_RelatorioCliente.ConfigurarGlyphsBotoes;
begin
...
procedure TForm_RelatorioCliente.FormCreate(Sender: TObject);
begin
  Color := COR_PAGE;
...
procedure TForm_RelatorioCliente.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  QueryCliente.Close;
  Action := caFree;
  Form_RelatorioCliente := nil;
end;

{ Navegacao por teclado }

procedure TForm_RelatorioCliente.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TForm_RelatorioCliente.EditFocoEnter(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_FOCO;
end;

procedure TForm_RelatorioCliente.EditFocoExit(Sender: TObject);
begin
  if Sender is TEdit then
    TEdit(Sender).Color := COR_EDIT_NORMAL;
end;

procedure TForm_RelatorioCliente.EditNumericoKeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TForm_RelatorioCliente.DBGridClienteDrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
var
  Grid: TDBGrid;
...
function TForm_RelatorioCliente.ValidarFaixaCodigo(out ACodigoDe,
...
procedure TForm_RelatorioCliente.AtualizarProgresso(Percentual: Integer);
begin
...
procedure TForm_RelatorioCliente.DestacarColunaOrdenacao;
var
  I: Integer;
  CampoOrdem: string;
begin
...
procedure TForm_RelatorioCliente.Pesquisar;
var
  Where, Sql: string;
...
procedure TForm_RelatorioCliente.BtnPesquisarClick(Sender: TObject);
begin
  Pesquisar;
...
procedure TForm_RelatorioCliente.BtnLimparClick(Sender: TObject);
begin
...
function TForm_RelatorioCliente.LocalizarArquivoFr3: string;
var
  PastaExe: string;
...
procedure TForm_RelatorioCliente.GravarRelatorioNoBanco(const ANome,
  ...
  try
    ...
    try
      ...
      Query.ParamByName('PDESCRICAO').AsString :=
        'Relatorio de Clientes com filtros';
...
function TForm_RelatorioCliente.CarregarRelatorioDoBanco(const ANome: string;
  Stream: TStream): Boolean;
var
  Query: TIBQuery;
begin
...
procedure TForm_RelatorioCliente.ExibirRelatorio;
var
...
begin
  ...
  try
    ...
    if not CarregarRelatorioDoBanco(NOME_RELATORIO_CLIENTE, Stream) then
    begin
      CaminhoFr3 := LocalizarArquivoFr3;
      if CaminhoFr3 = '' then
      begin
        MensagemDlg('Relatorio "' + NOME_RELATORIO_CLIENTE + '" nao ' +
          'encontrado na tabela RELATORIO e o arquivo ' + ARQUIVO_FR3 +
          ' nao foi localizado em disco.', mtWarning, [mbOK], 0);
        Exit;
      end;

      GravarRelatorioNoBanco(NOME_RELATORIO_CLIENTE, CaminhoFr3);
      if not CarregarRelatorioDoBanco(NOME_RELATORIO_CLIENTE, Stream) then
      begin
...
procedure TForm_RelatorioCliente.BtnImprimirClick(Sender: TObject);
begin
  if not QueryCliente.Active or QueryCliente.IsEmpty then
  begin
    MensagemDlg('Nao ha clientes para imprimir. Utilize o botao ' +
...
procedure TForm_RelatorioCliente.BtnSairClick(Sender: TObject);
begin
  Close;
end;
